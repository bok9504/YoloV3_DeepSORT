    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            if len(pts[track.track_id]) > 2:
                pts[track.track_id][-1]
                pointlen = np.sqrt(pow(pts[track.track_id][-1][0] - pts[track.track_id][-2][0], 2) + pow(pts[track.track_id][-1][1] - pts[track.track_id][-2][1], 2))
                realLen = (270*math.tan(math.radians(78.8/2))*2)*pointlen/width
                speed = realLen * FPS * 3.6
